...
$(function() {
  ...
  $(".folding-lst .folding-tit").on("click", function() {
    $(this).attr("title", "클릭시 해당 하위 내용 열림 (확장여부 : 축소)");
    ...
    if ($(this).next(".folding-cont").is(":hidden")) {
      ...
      $(this).parent().find(".folding-tit").attr("title", "클릭시 해당 하위 내용 접힘 (확장여부 : 확장)");
    }
    return false;
  });
  //folding-List 접근성
  $(".folding-lst > li .folding-cont .bul-lst01 > li:last-of-type > a").on("focusout", function() {
    $(this).parents().find(".folding-tit").attr("title", "클릭시 해당 하위 내용 열림 (확장여부 : 축소)");
